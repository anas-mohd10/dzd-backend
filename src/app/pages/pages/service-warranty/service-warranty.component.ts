...
@Component({
  ...
  styleUrls: ['./service-warranty.component.scss']
})
export class ServiceWarrantyComponent implements OnInit {
  ...
  editorConfig: AngularEditorConfig = {
    ...
    enableToolbar: true,
    showToolbar: true,
    placeholder: 'Enter service warranty here',
  ...
  ngOnInit(): void {
    this.ContentService.getContents().subscribe({
      next: (res: any) => {
        if (res?.errorCode == 0) {
          this.serviceWarranty.setValue(res?.result?.serviceWarranty)
        } else {
  ...
  manage() {
    if (!this.serviceWarranty.valid) {
      this.isSubmitted = true
      return
    }

    this.ContentService.manageContent({ serviceWarranty: this.serviceWarranty.value }).subscribe({
      next: (res: any) => {
        if (res?.errorCode == 0) {
          this.ngOnInit()
