...
export class BlogCategoriesComponent implements OnInit {
  ...
  delete(template: TemplateRef<any>, category: BlogCategory) {
  ...
  toggleStatus(event: { switchId: string, toggleState: boolean }) {
    this.BlogService.updateCategory(event.switchId, { isActive: event.toggleState }).subscribe({
      next: (res: any) => {
        if (res.errorCode == 0) {
  ...
  fetchCategory(categoryId: string) {
    this.BlogService.getCategoryDetails(categoryId).subscribe({
      next: (res: any) => {
        if (res.errorCode == 0) {
  ...
  updateCategory() {
    this.BlogService.updateCategory(this.selectedCategory?._id, this.form.value).subscribe({
      next: (res: any) => {
        if (res.errorCode == 0) {
          this.HotToastService.success(res.message)
          this.onSuccess()
  ...
  createCategory() {
    this.BlogService.createCategory(this.form.value).subscribe({
      next: (res: any) => {
        if (res.errorCode == 0) {
          this.HotToastService.success(res.message)
          this.onSuccess()
