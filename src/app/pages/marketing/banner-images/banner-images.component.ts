...
export class BannerImagesComponent implements OnInit {
  ...
  confirm() {
    this.BannerImagesService.deleteBannerImage(
      this.focusedBannerImage?.refid
    ).subscribe({
      next: (res: any) => {
        if (res?.errorCode == 0) {
  ...
  update() {
    this.BannerImagesService.updateBannerImage({
      ...this.form.value,
      refid: this.bannerImageDetails?.refid,
    }).subscribe({
      next: (res: any) => {
        if (res?.errorCode == 0) {
          this.Toast.success(res?.message);
    ...
    this.BannerImagesService.createBannerImage(this.form.value).subscribe({
      next: (res: any) => {
        if (res?.errorCode == 0) {
          ...
          this.duplicateRef = this.BsModalService.show(this.duplicateModal, {
            class: 'modal-sm modal-dialog-centered',
            ignoreBackdropClick: true,
          });
