...
export class AboutComponent implements OnInit {
  ...
  openLegacy(template: TemplateRef<void>) {
    this.legacyRef = this.BsModalService.show(template, { class: 'modal-sm modal-dialog-centered' })
  }
  ...
  openFeature(template: TemplateRef<void>) {
    this.featureRef = this.BsModalService.show(template, { class: 'modal-lg modal-dialog-centered' })
  }
  ...
  saveChanges() {
    ...
    this.AboutService.manageAbout(this.form.value).subscribe({
      next: (res: any) => {
        if (res?.errorCode == 0) {
          this.getAboutDetails()
          this.HotToastService.success(res?.message || "Changes saved successfully")
        } else {
