...
export class ShippingRulesComponent implements OnInit {
  ...
  states: Array<any> = []
  cities: Array<any> = []
  holidays: Array<any> = ["Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday"]
  ...
  onStateChange() {
    if (this.selectedStates.includes(this.stateInput.value)) {
      this.selectedStates = this.selectedStates.filter((state: any) => state != this.stateInput.value)
    ...
    for (let state of this.selectedStates) {
      let citiesInState = this.states.find((s: any) => s?.state == state)?.cities
  ...
  toggleHoliday(holiday: any) {
    if (this.selectedHolidays.includes(holiday)) {
      this.selectedHolidays = this.selectedHolidays.filter((h: any) => h != holiday)
  ...
  onCityChange() {
    if (this.selectedCities.includes(this.cityInput.value)) {
      this.selectedCities = this.selectedCities.filter((city: any) => city != this.cityInput.value)
  ...
  ngOnInit(): void {
    this.form = new FormGroup({
      ...
      minimumDay: new FormControl("", [Validators.required, Validators.pattern(/^\d+$/)]),
      maximumDay: new FormControl("", [Validators.required, Validators.pattern(/^\d+$/)]),
    })
      ...
      this.ShippingService.getShippingChargeDetails(chargeId).subscribe({
        next: (res: any) => {
          if (res?.errorCode == 0) {
            ...
            this.form.patchValue({ deliveryMethod: this.chargeDetails?.deliveryMethod?._id })
  ...
  onSubmit() {
    ...
    if (this.isEditMode) {
      this.ShippingService.updateShippingCharge({ _id: this.chargeDetails?._id, ...this.form.value }).subscribe({
        next: (res: any) => {
          if (res?.errorCode == 0) {
