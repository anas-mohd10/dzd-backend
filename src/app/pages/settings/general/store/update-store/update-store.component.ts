...
export class UpdateStoreComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (this.refid) {
      this.StoresService.getStoreDetails({ refid: this.refid }).subscribe(
        (res: any) => {
          if (res?.errorCode == 0) {
            ...
            this.form
              .get('countryCode')
              ?.setValue(res?.result?.contact?.countryCode);
            ...
            this.form
              .get('firstlane')
              ?.setValue(res?.result?.address?.firstlane);
            this.form
              .get('secondlane')
            ...
            this.form
              .get('startTime')
              ?.setValue(res?.result?.openingHours?.startTime);
            this.form
              .get('endTime')
              ?.setValue(res?.result?.openingHours?.endTime);
