...
@Component({
  ...
  styleUrls: ['./update-pickup.component.scss']
})
export class UpdatePickupComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.pickupId = this.ActivatedRoute.snapshot.params.pickupId || ''
    this.PickupService.details(this.pickupId).subscribe({
      next: (res: any) => {
        if (res?.errorCode == 0) {
  ...
  onSubmit() {
    ...
    this.PickupService.update(this.pickupId, this.form.value).subscribe({
      next: (res: any) => {
        if (res?.errorCode == 0) {
