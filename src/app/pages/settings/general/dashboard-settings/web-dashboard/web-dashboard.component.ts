...
export class WebDashboardComponent implements OnInit {
        ...
        for (let _item of this.dashboard) {
          switch (_item?.type) {
            case 'category':
              ...
              for (let _category of this.categoryDetails) this.categories.push(_category?.catid)
              break
          }
        }
        this.ChangeDetectorRef.markForCheck()
  ...
  searchCategory() {
    if (this.category.value) {
      this.CategoryService.findCategories({ keyword: this.category.value }).subscribe((res: any) => {
        if (res?.errorCode == 0) {
  ...
  selectCategory(category: any) {
    if (!this.categories.includes(category?.catid)) {
      if (this.categories.length < 7) {
        ...
        this.ToastrService.error('The operation could not be completed because the maximum limit has been reached')
      }
    } else {
      this.categoryDetails = this.categoryDetails.filter((item: any) => item.catid != category?.catid)
      this.categories = this.categories.filter((item: any) => item != category?.catid)
    }
  }
  ...
  manageCategory() {
    if (this.categories.length > 0) {
      ...
      this.dashboard[this.categoryIndex] = {
        ...
        title: { text: "Browse All Categories", color: "0xff000000", "font-weight": 8 },
        button: {
          label: { text: "View More", color: "0xff00bdab", font: "Manrope", "font-style": 0, "font-size": 16, "font-weight": 4 },
          bgcolor: ["0xff00bdab"],
          type: 1
        },
        Displaystatus: true
      ...
      this.ToastrService.error('The operation could not be completed because a minimum of 1 item is required')
    }
  }
  ...
  editCollection(slug: any) {
    ...
    this.CollectionService.getCollectionBySlug(slug).subscribe((res: any) => {
      if (res?.errorCode == 0) {
        ...
        for (let _key of Object.keys(res?.result[0])) this.collectionForm.get(_key)?.setValue(res?.result[0][_key])
        this.productDetails = res?.result[0]?.products
        for (let _product of this.productDetails) this.products.push(_product?._id)
        this.ChangeDetectorRef.markForCheck()
      }
    })
  }
    ...
    switch (this.isCollectionEdit) {
      ...
      case false:
        this.CollectionService.addCollection(payload).subscribe((res: any) => {
          if (res?.errorCode == 0) {
            this.ProductService.getProductWebData({ products: res?.result?.products }).subscribe((resData: any) => {
  ...
  addCollectionToDashboard(collection: any, products: any) {
    this.dashboard.push({
      cover: "",
      title: { text: collection?.name, color: collection?.style?.color, "font-weight": 8 },
      subtitle: { text: collection?.subname, color: collection?.style?.color, "font-weight": 8 },
      product_items: products,
      button: {
        label: { text: "View More", color: "#ffffff", "font-style": 0, "font-size": 16, "font-weight": 4 },
        bgcolor: ["0xFFbe1e2d"],
        type: 1,
        params: { action: "", colid: collection?.colid, slug: collection?.slug },
        action: ""
      },
      type: 'product',
      Displaystatus: true
