...
export class UpdateHeadComponent implements OnInit {
  ...
  getProductHead() {
    this.ProductHeadService.productHeadDetails(this?._id || '').subscribe(
      (res: any) => {
        if (res?.errorCode == 0) {
          ...
          this.productheadform
            .get('cod')
            ?.setValue(this.basicdetails?.cod?.isPresent);
          this.productheadform
            .get('codCharge')
            ?.setValue(this.basicdetails?.cod?.value);
          this.productheadform
            .get('shippingMethod')
            ?.setValue(this.basicdetails?.shipping?.method);
          this.basicdetails?.shipping?.method == 'Paid'
            ? (this.isShipping = true)
            : (this.isShipping = false);
          this.basicdetails?.return?.isPresent == true
            ? (this.isReturn = true)
            : (this.isReturn = false);
          this.basicdetails?.cod?.isPresent == true
            ? (this.isCod = true)
            : (this.isCod = false);
          this.productheadform
            .get('shippingCost')
            ?.setValue(this.basicdetails?.shipping?.value);
          this.productheadform
            .get('returnable')
            ?.setValue(this.basicdetails?.return?.isPresent);
          this.productheadform
            .get('returnDays')
            ?.setValue(this.basicdetails?.return?.value);
          this.productheadform
            .get('isActive')
            ?.setValue(this.basicdetails?.isActive);
          this.productheadform
            .get('isArchive')
            ?.setValue(this.basicdetails?.isArchive);
          this.CategoryService.getSubCategoriesDetails(
            this.basicdetails?.parentCategory?.id
          ).subscribe((res: any) => {
            if (res?.errorCode == 0) {
