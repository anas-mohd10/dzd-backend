...
export class CategoryComponent implements OnInit {
  ...
  openAttributeModal(template: TemplateRef<any>, category: any) {
    ...
    this.attributeModalRef = this.BsModalService.show(template, {
  ...
  openManageAttributeModal(template: TemplateRef<any>, attribute: any) {
    ...
    this.manageModalRef = this.BsModalService.show(template, {
      class: 'modal-lg modal-dialog-centered',
    });
  ...
  getAttributeValue(type: any, event: any) {
    switch (type) {
      case 'text':
        if (!this.texts.includes(event.target.value) && event.target.value)
          this.texts.push(event.target.value);
        this.text.setValue('');
        break;
      case 'color':
        if (!this.colors.includes(event.target.value) && event.target.value)
  ...
  onSubmit() {
    ...
    if (!this.attributeForm.valid || this.values.length == 0) {
      this.HotToastService.error(
        'Invalid form submission, kindly check all the fields and submit again.'
      );
      return;
    }
